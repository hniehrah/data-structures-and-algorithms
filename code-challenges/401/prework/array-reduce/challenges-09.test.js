...
const reversedString = (arr) => {
  return arr.split('').reduce((acc, current) => {
    current += acc;
    return current;
  });
};
...
const extractStat = (statName, arr) => {
  return arr.reduce((acc, current) => {
    if (statName === current.stat.name) acc.push(current);
    return acc;
  });
};
...
const extractChildren = (arr) => {
  const output = arr.filter(element => {
    if (element.name.includes('a')) return element;
  });
  return output.reduce((acc, current, index) => {
    if (current.children) current.children.forEach(e => acc.push(e));
    return acc;
  }, []);
};
