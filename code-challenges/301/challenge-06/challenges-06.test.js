...
const getHouses = (arr) => {
  let houses = [];
  for (var i in arr) houses.push(arr[i].house);
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  for (let i in Object.values(arr)){
    if (character === Object.values(arr[i])[0]){
...
const hasChildrenEntries = (arr, character) => {
  for (let i in Object.entries(arr)){
...
const houseSize = (arr) => {
  const sizes = [];
  arr.forEach(entry => {
    let newEntry = {house: entry.house, members: entry.children.length + 1};
    if (entry.spouse) newEntry.members++;
    sizes.push(newEntry);
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(entry => {
    let newEntry = {house: entry.house, members: entry.children.length + 1};
    if (entry.spouse && !deceasedSpouses.includes(entry.spouse)) newEntry.members++;
