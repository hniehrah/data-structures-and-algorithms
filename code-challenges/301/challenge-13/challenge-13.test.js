...
const allHappy = (arr) => {
  let initialArr = arr.map(el => el.includes(':)'));
  if (initialArr.includes(false)) return false;
  else return true;
};
...
const findEvery = (arr, target) => {
  let initialArr = arr.map(el => el.includes(target));
  if (initialArr.includes(false)) return false;
  else return true;
};
...
const unenrollBrook = (arr) => {
  return arr.map(el => {
    return el.filter(entry => entry.includes('Brook') === false)
  });
};
/* ------------------------------------------------------------------------------------------------
...
const sortByDay = (arr) => {
  return daysOfWeek.map(day => arr.filter(el => el.includes(day)));
};
...
const characterByIndex = (arr) => {
  return arr.map((el, index) => el.charAt(index));
};
