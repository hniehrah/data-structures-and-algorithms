...
// Other solutions I practiced with that work:
// const notInFirstArray = (forbiddenValues, arr) => {
//   return arr.filter(e => {
//     return forbiddenValues.indexOf(e) < 0;
//   })
// };
...
//   arr.filter(e => {
//     for (var i in arr){
//       for (var j in forbiddenValues){
//         if (arr[i] === forbiddenValues[j]) arr.splice(i, 1);
//       }
//     }
//   })
//   return arr;
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(e => {
    if (minBaseStat < e.baseStat) return e;
  });
};
...
const getStatName = (arr, minBaseStat) => {
  let output = [];
  arr.filter(e => {
    if (minBaseStat < e.baseStat) output.push(e.stat.name);
  });
  return output;
};
